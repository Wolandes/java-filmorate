...
COMMENT ON TABLE PUBLIC.FEED IS 'Содержит информацию о событиях';
COMMENT ON COLUMN PUBLIC.FEED.USER_ID IS 'Пользователь, с которым связано событие';
COMMENT ON COLUMN PUBLIC.FEED.EVENT_TYPE IS 'С чем связано событие: лайк, друг, обзор';
COMMENT ON COLUMN PUBLIC.FEED.OPERATION IS 'Операция события: добавление, удаление, обновление';
COMMENT ON COLUMN PUBLIC.FEED.ENTITY_ID IS 'Идентификатор сущности, с которой произошло событие';
...
(
  ...
  CONSTRAINT FK_REVIEW_FILMS FOREIGN KEY (FILM_ID) REFERENCES PUBLIC.FILMS (ID) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT FK_REVIEW_USER FOREIGN KEY (USER_ID) REFERENCES PUBLIC.USERS (ID) ON DELETE RESTRICT ON UPDATE CASCADE
);
COMMENT ON TABLE PUBLIC.REVIEWS IS 'Содержит информацию об отзывах';
COMMENT ON COLUMN PUBLIC.REVIEWS.ID IS 'Уникальный идентификатор';
COMMENT ON COLUMN PUBLIC.REVIEWS.CONTENT IS 'Текст отзыва';
COMMENT ON COLUMN PUBLIC.REVIEWS.FILM_ID IS 'Фильм, ссылка на поле ID таблицы FILMS';
COMMENT ON COLUMN PUBLIC.REVIEWS.USER_ID IS 'Пользователь, ссылка на поле ID таблицы USERS';
COMMENT ON COLUMN PUBLIC.REVIEWS.IS_POSITIVE IS 'Тип отзыва';

CREATE TABLE IF NOT EXISTS PUBLIC.REVIEW_USEFUL
(
  REVIEW_ID INTEGER NOT NULL,
  USER_ID INTEGER NOT NULL,
  WEIGHT INTEGER NOT NULL,
  CONSTRAINT PK_REVIEW_USEFUL PRIMARY KEY (REVIEW_ID, USER_ID),
  CONSTRAINT FK_REVIEW_USEFUL_REVIEW FOREIGN KEY (REVIEW_ID) REFERENCES PUBLIC.REVIEWS (ID) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT FK_REVIEW_USEFUL_USER FOREIGN KEY (USER_ID) REFERENCES PUBLIC.USERS (ID) ON DELETE RESTRICT ON UPDATE CASCADE
);
COMMENT ON TABLE PUBLIC.REVIEW_USEFUL IS 'Содержит информацию о лайках и дизлайках пользователей на отзывы';
COMMENT ON COLUMN PUBLIC.REVIEW_USEFUL.REVIEW_ID IS 'Отзыв, ссылка на поле ID таблицы REVIEWS';
COMMENT ON COLUMN PUBLIC.REVIEW_USEFUL.USER_ID IS 'Пользователь, ссылка на поле ID таблицы USERS';
COMMENT ON COLUMN PUBLIC.REVIEW_USEFUL.WEIGHT IS 'Значение, 1 соответствует лайку, -1 - соответствует дизлайку';
