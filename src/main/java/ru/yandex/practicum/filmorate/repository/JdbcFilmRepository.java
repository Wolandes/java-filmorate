...
public class JdbcFilmRepository extends BaseRepository implements FilmRepository {
    ...
    private static final String findAllQuery = "SELECT f.*, m.name AS mpa_name FROM Film f JOIN Mpa_Rating m ON f.mpa_rating_id = m.id";
    ...
    public Film addFilm(Film postFilm) {
        long id = insert("INSERT INTO Film (name, description, releaseDate, duration, mpa_rating_id) VALUES (?, ?, ?, ?, ?)",
                postFilm.getName(), postFilm.getDescription(), postFilm.getReleaseDate(), postFilm.getDuration(), postFilm.getMpa().getId());
        postFilm.setId(id);
        for (Genre genre : postFilm.getGenres()) {
            if (genre.getId() > 0) {
                update("INSERT INTO Film_Genre (film_id, genre_id) VALUES (?, ?)", id, genre.getId());
            }
        }
        ...
        public Film updateFilm (Film putFilm){
            update("UPDATE Film SET name = ?, description = ?, release_date = ?, duration = ?, mpa_rating_id = ? WHERE id = ?",
                    putFilm.getName(), putFilm.getDescription(), putFilm.getReleaseDate(), putFilm.getDuration(), putFilm.getMpa().getId(), putFilm.getId());
        ...
        public List<Film> getPopularFilms ( int count){
            return jdbc.query(
                    "SELECT f.*, m.name AS mpa_name, COUNT(fl.user_id) AS likes " +
                            "FROM Film f " +
